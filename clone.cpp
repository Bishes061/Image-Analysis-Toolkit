...
int guiBlockSize = 4;            // 2^4 = 16 default
int guiDetailThreshold = 97;     // default = 9.7
int guiMinClusterSize = 3;
int guiQuantizeLevel = 16;       // default = 16
int guiMaxImageSize = 1024;       // default = 1024
int showQuantized = 0;           // Checkbox state
...
string blockToKey(const Mat& block, Mat& smallOut, int quantDiv) {
    Mat gray, small;
    cvtColor(block, gray, COLOR_BGR2GRAY);
    resize(gray, small, Size(4, 4));
    smallOut = small.clone();
    ...
    for (int i = 0; i < small.rows; i++) {
        for (int j = 0; j < small.cols; j++) {
            ss << (int)(small.at<uchar>(i, j) / quantDiv) << ",";
        }
    }
    return ss.str();
}
...
void processAndShow() {
    int blockSize = pow(2, max(guiBlockSize, 3));
    double detailThreshold = guiDetailThreshold / 10.0;
    int minClusterSize = max(guiMinClusterSize, 1);
    int quantizeDivisor = max(1, 256 / max(guiQuantizeLevel, 1));

    Mat img = imread("path of your image");
    if (img.empty()) {
        cout << "Could not open image" << endl;
        return;
    }

    if (img.cols > guiMaxImageSize || img.rows > guiMaxImageSize) {
        double scale = guiMaxImageSize / (double)max(img.cols, img.rows);
    ...
    for (int y = 0; y <= img.rows - blockSize; y += 4) {
        for (int x = 0; x <= img.cols - blockSize; x += 4) {
            Rect roi(x, y, blockSize, blockSize);
            Mat block = img(roi);
            ...
            Mat compressed;
            string key = blockToKey(block, compressed, quantizeDivisor);

            if (blockMap.find(key) != blockMap.end()) {
                Point orig = blockMap[key];
                Point curr = Point(x, y);
                if (euclideanDistance(orig, curr) < 20)
                    continue;
    ...
    stringstream ss;
    ss << "Block Size: " << blockSize << "  Detail Threshold: " << detailThreshold
       << "  Cluster Size: " << minClusterSize << "  Quant Levels: " << guiQuantizeLevel
       << "  Max Img: " << guiMaxImageSize;

    putText(result, ss.str(), Point(10, 25), FONT_HERSHEY_SIMPLEX, 0.4, Scalar(255, 255, 255), 1);
...
int main() {
    namedWindow("Clone Detector", WINDOW_AUTOSIZE);
    createTrackbar("Block Size (2^n)", "Clone Detector", &guiBlockSize, 6, onTrackbarChange);
    createTrackbar("Min Detail (x0.1)", "Clone Detector", &guiDetailThreshold, 255, onTrackbarChange);
    createTrackbar("Min Cluster Size", "Clone Detector", &guiMinClusterSize, 20, onTrackbarChange);
    createTrackbar("Quantize Levels", "Clone Detector", &guiQuantizeLevel, 64, onTrackbarChange);
    createTrackbar("Max Image Size", "Clone Detector", &guiMaxImageSize, 2000, onTrackbarChange);
    createTrackbar("Show Quantized", "Clone Detector", &showQuantized, 1, onTrackbarChange);

    processAndShow();

    while (waitKey(30) != 27); // ESC to exit
    return 0;
}
